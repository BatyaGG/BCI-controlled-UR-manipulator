...
<p align="center"> 
<img src="https://writelatex.s3.amazonaws.com/wndkfttkbxvm/uploads/982/..; width="70%"> 
</p> 
...
The above figure A) shows control interface during the robot performing movements.
First 3 rows represent 3 axis of end-effector movements _X_, _Y_ and _Z_ axises respectively. The 4th row refer to the control of the gripper while _C_ and _O_ indicates open and close actions of the gripper which are also controlled by imagining the left and right hand movements respectively. The left column with '-' sign and right column with '+' sign indicate positive or negative movement on selected axis which correspond to the left hand motor imagery and right hand motor imagery respectively and will be indicated on control interface by coloring particular block in first or second columns into green color. The green line under the row indicate on which axis a gripper will perform a movement. This indicator is iterating consequently down and at the bottom up row by row, each time user imagining legs' movement and followed by audio providing the information about current active axis. Using the audio cue allows user to be independent from screen, and follow of the robot movement only. In B part of the Figure \ref{fig:con_interface}, shown UR5 robot manipulator with indicated on gripper position the control axises where green, red and blue related to the _X_, _Y_ and _Z_ axises of the movement. \newline In this particular manner user will be able to control the gripper manipulator without necessity to look at the screen and constantly be able to control robots movements.
